<script>
  ...
  const productList = Object.values(products).filter((el) =>
    el.id.includes(name.toLowerCase().slice(0, 4))
  );
  const category = Object.values(categories).filter(
    (el) => el.name.replaceAll(' ', '') === name
  );
  ...
  <div class="container">
    <div class="products-head">
      <button class="back-btn" on:click={goBack}
        ><img
          src="/src/lib/images/left-arrow.png"
          alt="뒤로가기 버튼"
        /></button
      >
